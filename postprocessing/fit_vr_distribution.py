...
from scipy.stats import beta, rv_histogram, skewnorm
...
for sim in sim_dirs:
    ...
    #plt.savefig('../figures/log_vr_fit_with_skewnorm/vr_{}.pdf'.format(str(mbh)+'_'+str(abh)+'_'+str(mdisk_init)))
    ...
    with warnings.catch_warnings():
        ...
        a, loc, scale = skewnorm.fit(draws)
        #a, b, loc, scale = beta.fit(draws, floc=0)
    #res = minimize(func, x0=[a, b/scale])
    res = minimize(func, x0=[a, loc, scale])
    #a_fit, b_fit = res.x
    a_fit, loc_fit, scale_fit = res.x
    
    #with open('../paper_data/vr_beta_fit_parameters.dat', 'a') as f:
    with open('../paper_data/log_vr_skewnorm_fit_parameters.dat', 'a') as f:
        #print('{0:.2f} & {1:.2f} & {2:.3g} & {3:.2f} & {4:.2f} & {5:.3f} & {6:.3f} \\\\'.format(mbh, abh, mdisk_init, ye_init, s_init, a_fit, b_fit), file=f)
        print('{0:.2f} & {1:.2f} & {2:.3g} & {3:.2f} & {4:.2f} & {5:.3f} & {6:.3f} & {7:.3f} \\\\'.format(mbh, abh, mdisk_init, ye_init, s_init, a_fit, loc_fit, scale_fit), file=f)
    f.close()
    #x = np.linspace(beta.ppf(0.001, a_fit, b_fit, loc=0), beta.ppf(0.999, a_fit, b_fit, loc=0), 100)
    x = np.linspace(skewnorm.ppf(0.001, a_fit, loc=loc_fit, scale=scale_fit), skewnorm.ppf(0.999, a_fit, loc=loc_fit, scale=scale_fit), 100)
    #plt.plot(x, beta.pdf(x, a_fit, b_fit, loc=0), 'r-')
    plt.plot(x, skewnorm.pdf(x, a_fit, loc=loc_fit, scale=scale_fit), 'r-')
    plt.hist(vr, bins=100, density=True, weights=mass_weights)
    #plt.savefig('../figures/vr_fit_with_beta/vr_{}.pdf'.format(str(mbh)+'_'+str(abh)+'_'+str(mdisk_init)))
    plt.savefig('../figures/log_vr_fit_with_skewnorm/vr_{}.png'.format(str(mbh)+'_'+str(abh)+'_'+str(mdisk_init)))
    plt.close()
    #print(a_fit, b_fit)   
    print(func((a_fit, loc_fit, scale_fit)))
